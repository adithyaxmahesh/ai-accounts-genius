...
const IncomeStatement = () => {
  ...
  const { data: incomeStatements, isLoading } = useQuery({
    queryKey: ["incomeStatements", session?.user.id],
    queryFn: async () => {
    ...
    <div className="min-h-screen bg-background p-6 space-y-6">
      ...
      <div className="grid gap-6">
        {incomeStatements?.map((statement) => (
          <Card key={statement.id} className="p-6">
            <div className="flex justify-between items-start">
              ...
              <p className={`text-lg font-semibold ${statement.type === 'revenue' ? 'text-green-600' : 'text-red-600'}`}>
        ...
        {(!incomeStatements || incomeStatements.length === 0) && (
          <p className="text-center text-muted-foreground">No income statements found</p>
        )}
      </div>
    </div>
  );
